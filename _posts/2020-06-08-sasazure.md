...
Many users of SAS have spent their time using what is commonly called "PC SAS."  They author and run SAS code on a local personal machine.  The authoring SAS code choices have evolved, with the most recent being SAS Studio, a browser-based interface. It is now even easier to conceptualize moving the running of a SAS program to virtually anywhere.  This post starts with this backdrop and shows moving the SAS platform to Microsoft Azure and using SAS Studio to work with files in the Azure infrastructure.  
